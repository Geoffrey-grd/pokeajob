...
class Student extends User {
    ...
    public function create_student($email, $password, $name, $last_name, $pilot, $role) {
        ...
        $stmt = $this->conn->prepare("Select pilot.school FROM pilot WHERE pilot.id_user = ?");
        $stmt->execute([$pilot]);
        ...
        return $stmt->execute([$id_user, $name, $last_name, $school, $pilot]);
    }

    public function getStudentInformations($id_user) {
        $stmt = $this->conn->prepare("SELECT user.email, student.name, student.last_name, pilot.school, student.description, student.profile_pic_path, pilot.name AS pilot_name, pilot.last_name AS pilot_last_name
